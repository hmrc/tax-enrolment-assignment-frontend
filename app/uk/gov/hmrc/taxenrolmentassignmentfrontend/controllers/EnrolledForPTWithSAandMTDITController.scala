...
import uk.gov.hmrc.taxenrolmentassignmentfrontend.views.html.EnrolledForPTWithSaAndMtditPage

import javax.inject.{Inject, Singleton}
import scala.concurrent.ExecutionContext

@Singleton
class EnrolledForPTWithSAandMTDITController @Inject() (
  authAction: AuthAction,
  accountMongoDetailsAction: AccountMongoDetailsAction,
  mcc: MessagesControllerComponents,
  multipleAccountsOrchestrator: MultipleAccountsOrchestrator,
  val logger: EventLoggerService,
  enrolledForPTPage: EnrolledForPTWithSaAndMtditPage,
        ...
        case Right(accountDetails) =>
          Ok(
            enrolledForPTPage(
              ...
              routes.EnrolledForPTWithSAandMTDITController.continue
            )
          )
        case Left(error)           =>
          errorHandler.handleErrors(error, "[EnrolledForPTWithSAandMTDITController][view]")(request, implicitly)
      }
    }
    ...
    authAction.andThen(accountMongoDetailsAction).async { implicit request =>
      logger.logEvent(
        logRedirectingToReturnUrl(
          ...
          "[EnrolledForPTWithSAAndMTDITController][continue]"
        )
      )
