...
case class UserDetailsFromSession(credId: String, nino: String, hasPTEnrolment: Boolean, hasSAEnrolment: Boolean)
case class RequestWithUserDetails[A](request: Request[A], userDetails: UserDetailsFromSession) extends WrappedRequest[A](request)
trait AuthIdentifierAction extends ActionBuilder[RequestWithUserDetails, AnyContent] with ActionFunction[Request, RequestWithUserDetails]
...
class AuthAction @Inject()(override val authConnector: AuthConnector,
  ...
  override def invokeBlock[A](request: Request[A], block: RequestWithUserDetails[A] => Future[Result]): Future[Result] = {
    ...
    authorised(AuthProviders(GovernmentGateway) and ConfidenceLevel.L200).retrieve(nino and credentials and allEnrolments) {
      case Some(nino) ~ Some(credentials) ~ enrolments =>
        ...
        val userDetails = UserDetailsFromSession(credentials.providerId, nino, hasPTEnrolment, hasSAEnrolment)

        block(RequestWithUserDetails(request, userDetails))

      case x => logger.warn(s"[AuthAction][invokeBlock] session missing credential or NINO field for uri: ${request.uri}")
