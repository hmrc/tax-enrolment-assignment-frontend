  ...
  "invokeBlock" should {
    "call to auth for a valid throttle scenario and redirect the user to their redirect url" in {
      ...
      stubPutWithRequestBody(
        url ="/auth/enrolments",
        status = OK,
        requestBody = Json.toJson(exampleRequestBelowThreshold.userDetails.enrolments.enrolments + newEnrolment(exampleRequestBelowThreshold.userDetails.nino))(EnrolmentsFormats.writes).toString,
        responseBody = "")

      val res = throttleAction.invokeBlock(exampleRequestBelowThreshold, exampleControllerFunction)
    ...
    "NOT call to auth for a invalid valid throttle scenario (above threshold) and not redirect to users redirect url " in {
      val res = throttleAction.invokeBlock(exampleRequestAboveThreshold, exampleControllerFunction)
