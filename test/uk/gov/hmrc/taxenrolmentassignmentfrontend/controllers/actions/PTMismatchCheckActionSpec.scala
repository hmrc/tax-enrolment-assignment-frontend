...
class PTMismatchCheckActionSpec extends TestFixture {
  ...
  def harness[A](block: RequestWithUserDetailsFromSession[_] => Future[Result])(implicit request: RequestWithUserDetailsFromSession[A]): Future[Result] = {
    lazy val actionProvider = mock[PTMismatchCheckAction]
    ...
    "a user does not have a HMRC-PT enrolment" should {
      "not make any changes to the existing enrolments" in {
        val block: RequestWithUserDetailsFromSession[_] => Future[Result] = userRequest =>
          Future.successful(Ok(s"Person Details: ${userRequest.userDetails}"))

        val action = harness(block)(request)
