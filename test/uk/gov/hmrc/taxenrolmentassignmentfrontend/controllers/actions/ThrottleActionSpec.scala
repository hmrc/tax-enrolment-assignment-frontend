...
class ThrottleActionSpec extends TestFixture {
  ...
  val action = new ThrottleAction(mockThrottlingService, testBodyParser, errorHandler)
  val exampleRequest = RequestWithUserDetailsFromSessionAndMongo(
    FakeRequest(),
    UserDetailsFromSession("123", "nino", "gID", Enrolments(Set(Enrolment("foo"))), false, false),
    "sesh",
    AccountDetailsFromMongo(PT_ASSIGNED_TO_CURRENT_USER, "redirectURL")
  )
  val exampleControllerFunction = (r: RequestWithUserDetailsFromSessionAndMongo[_]) => Future.successful(Ok("got through"))
  "invokeBlock" should {
