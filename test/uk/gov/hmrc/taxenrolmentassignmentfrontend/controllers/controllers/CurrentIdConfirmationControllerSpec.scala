...
import uk.gov.hmrc.taxenrolmentassignmentfrontend.controllers.CurrentIdConfirmationController
...
class CurrentIdConfirmationControllerSpec extends TestFixture {
  val view: CurrentIdConfirmation = app.injector.instanceOf[CurrentIdConfirmation]
  val controller =
    new CurrentIdConfirmationController(mockAuthAction, view, mcc)
  ...
  "view" should {
    "present the current ID confirmation page" in {
      ...
      contentAsString(result) should include("currentIdConfirmation.title")
    }
  }
}