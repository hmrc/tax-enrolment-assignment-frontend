...
import uk.gov.hmrc.taxenrolmentassignmentfrontend.messages.PTEnrolmentOtherAccountMessagesEnrolmentGGLoggedInOL
import uk.gov.hmrc.taxenrolmentassignmentfrontend.models.MFADetails
import uk.gov.hmrc.taxenrolmentassignmentfrontend.views.html.PTEnrolmentOnGGAccountLoggedInOL

class PTEnrolmentOnGGAccountLoggedInOLSpec extends ViewSpecHelper {

  lazy val view: PTEnrolmentOnGGAccountLoggedInOL =
  ...
  val htmlWithSA =
    view(ptEnrolmentDataModelOL(Some(CREDENTIAL_ID_1), ptAccountDetails = testAccountDetailsWithSA))(
      FakeRequest(),
      testMessages
    )
  ...
  val htmlWithSAAndMtdit =
    view(ptEnrolmentDataModelOL(Some(CREDENTIAL_ID_1), ptAccountDetails = testAccountDetailsWithSA, hasMtdit = true))(
      FakeRequest(),
      testMessages
    )
  ...
  val htmlNoEmail     =
    view(
      ptEnrolmentDataModelOL(Some(NO_EMAIL_USER_ID), ptAccountDetails = accountDetailsWithNoEmail)
    )(FakeRequest(), testMessages)
  val documentNoEmail = doc(htmlNoEmail)

  "PTEnrolmentOnAnotherAccount" should {
    "have the expected title" in {
      documentWithSA.title() shouldBe PTEnrolmentOtherAccountMessagesEnrolmentGGLoggedInOL.title
    }
    "have the expected heading" in {
      documentWithSA
        .getElementsByClass(Selectors.heading)
        .text shouldBe PTEnrolmentOtherAccountMessagesEnrolmentGGLoggedInOL.heading
    }
    ...
    "have a body" that {
      ...
      "has the expected text" in {
        textElement.text shouldBe PTEnrolmentOtherAccountMessagesEnrolmentGGLoggedInOL.text1
      }
    }
    ...
    "contains a link for if userId not recognised" that {
      ...
      "has the correct text" in {
        documentWithSA.body.text() should include(PTEnrolmentOtherAccountMessagesEnrolmentGGLoggedInOL.notMyUserId)
      }
