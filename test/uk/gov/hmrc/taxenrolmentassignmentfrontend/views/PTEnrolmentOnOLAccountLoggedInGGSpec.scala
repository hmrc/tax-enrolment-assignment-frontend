...
import uk.gov.hmrc.taxenrolmentassignmentfrontend.messages.PTEnrolmentOtherAccountMessagesEnrolmentOLLoggedInGG
import uk.gov.hmrc.taxenrolmentassignmentfrontend.models.MFADetails
import uk.gov.hmrc.taxenrolmentassignmentfrontend.views.html.PTEnrolmentOnOLAccountLoggedInGG

class PTEnrolmentOnOLAccountLoggedInGGSpec extends ViewSpecHelper {

  lazy val view: PTEnrolmentOnOLAccountLoggedInGG =
    inject[PTEnrolmentOnOLAccountLoggedInGG]
  ...
  val htmlWithSA =
    view(ptEnrolmentDataModel(Some(CREDENTIAL_ID_1), ptAccountDetails = testAccountDetailsWithSAOL))(
      FakeRequest(),
      testMessages
    )
  ...
  val htmlWithSAAndMtdit =
    view(ptEnrolmentDataModel(Some(CREDENTIAL_ID_1), ptAccountDetails = testAccountDetailsWithSA, hasMtdit = true))(
      FakeRequest(),
      testMessages
    )
  ...
  val html     =
    view(ptEnrolmentDataModel(None, ptAccountDetails = testAccountDetails))(
      FakeRequest(),
      testMessages
    )
  ...
  "PTEnrolmentOnAnotherAccount" should {
    "have the expected title" in {
      documentWithSA.title() shouldBe PTEnrolmentOtherAccountMessagesEnrolmentOLLoggedInGG.title
    }
    "have the expected heading" in {
      documentWithSA
        .getElementsByClass(Selectors.heading)
        .text shouldBe PTEnrolmentOtherAccountMessagesEnrolmentOLLoggedInGG.heading
    }
    ...
    "have a body" that {
      ...
      "has the expected text" in {
        textElement.text shouldBe PTEnrolmentOtherAccountMessagesEnrolmentOLLoggedInGG.text1
      }
    }
    ...
    "contains a link for if userId not recognised" that {
      ...
      "has the correct text" in {
        documentWithSA.body.text() should include(PTEnrolmentOtherAccountMessagesEnrolmentOLLoggedInGG.notMyUserId)
      }
      "contains a link to report suspicious Id" in {
        textElement
          .select("a.govuk-link")
          .get(2)
          .attr("href") should include(PTEnrolmentOtherAccountMessagesEnrolmentOLLoggedInGG.fraudReportingUrl)
      }
    }
